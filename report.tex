...
\end{equation}
with source and initial state as described in section TODO. A test for convergence rate may look something like
\begin{lstlisting}[language=Python]
...
&\frac{1}{\Delta y^2} \left( c_{i,j+1/2} (u_{i,j+1} - u_{i,j}) - c_{i,j-1/2}(u_{i,j} - u_{i,j-1}) \right) + f^n_{i,j},
\end{align*}
...
OpenGL (Open Graphics Library, OGL) is an API that can be used to communicate with the graphics card. In a basic OGL-program, we define a window that has a width and height (measured in pixels) and offers a set of objects (points, triangles, quads, polygons etc) that can be rendered in the window.
\subsection*{Rendering points and triangles}
If we have 10 points at position $r_i=(x_i,y_i,z_i)$ saved in a matrix, we can render these points with the following code
\begin{lstlisting}
glBegin(GL_VERTEX);
for (int i=0; i<10; i++) {
...
glEnd();
\end{lstlisting}
Usually we want to render a surface rather than points, so we use one of the other \textit{primitives} such as \textit{GL\_TRIANGLES}. If we call \textit{glBegin(GL\_TRIANGLES);} instead, OGL expects three points defining a triangle, a \textit{face}. We can then render our whole grid by dividing it into small triangles.
\end{document}
